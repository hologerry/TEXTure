...
class StableDiffusion(nn.Module):
    def __init__(
        ...
        self.vae = AutoencoderKL.from_pretrained(model_name, subfolder="vae", use_auth_token=self.token).to(
            self.device
        )
        ...
        if self.use_inpaint:
            self.inpaint_unet = UNet2DConditionModel.from_pretrained(
                "stabilityai/stable-diffusion-2-inpainting", subfolder="unet", use_auth_token=self.token
            ).to(self.device)
    ...
    def img2img_single_step(self, text_embeddings, prev_latents, depth_mask, step, guidance_scale=100):
        ...
        def sample(prev_latents, depth_mask, step):
            ...
            latent_model_input = self.scheduler.scale_model_input(latent_model_input, step)  # NOTE: This does nothing

            latent_model_input_depth = torch.cat([latent_model_input, depth_mask], dim=1)
            # predict the noise residual
            with torch.no_grad():
                noise_pred = self.unet(latent_model_input_depth, step, encoder_hidden_states=text_embeddings)["sample"]
    ...
    def img2img_step(
        ...
        def sample(
            latents, depth_mask, strength, num_inference_steps, update_mask=None, check_mask=None, masked_latents=None
        ):
                    ...
                    if is_inpaint_iter:
                        ...
                        with torch.no_grad():
                            noise_pred_inpaint = self.inpaint_unet(
                                latent_model_input_inpaint, t, encoder_hidden_states=text_embeddings
                            )["sample"]
    ...
    def produce_latents(
        ...
        if latents is None:
            # Last chanel is reserved for depth
            latents = torch.randn(
                (text_embeddings.shape[0] // 2, self.unet.in_channels - 1, height // 8, width // 8), device=self.device
            )
            timesteps = self.scheduler.timesteps
        else:
            # Strength has meaning only when latents are given
    ...
    def prompt_to_img(
        ...
        depth_mask = F.interpolate(
            depth_mask.unsqueeze(1), size=(height // 8, width // 8), mode="bicubic", align_corners=False
        )
