...
class TEXTure:
    def __init__(self, cfg: TrainConfig):
        ...
        self.back_im = (
            torch.Tensor(np.array(Image.open(self.cfg.guide.background_img).convert("RGB")))
            .to(self.device)
            .permute(2, 0, 1)
            / 255.0
        )
    ...
    def init_mesh_model(self) -> nn.Module:
        ...
        logger.info(f"Loaded Mesh, #parameters: {sum([p.numel() for p in model.parameters() if p.requires_grad])}")
        logger.info(model)
        return model
    ...
    def init_dataloaders(self) -> Dict[str, DataLoader]:
        ...
        dataloaders = {"train": init_train_dataloader, "val": val_loader, "val_large": val_large_loader}
        return dataloaders
    ...
    def evaluate(self, dataloader: DataLoader, save_path: Path, save_as_video: bool = False):
        ...
        if save_as_video:
            ...
            dump_vid = lambda video, name: imageio.mimsave(
                save_path / f"step_{self.paint_step:05d}_{name}.mp4", video, fps=25, quality=8, macro_block_size=1
            )
    ...
    def paint_viewpoint(self, data: Dict[str, Any]):
        ...
        meta_output = self.mesh_model.render(
            background=torch.Tensor([0, 0, 0]).to(self.device), use_meta_texture=True, render_cache=render_cache
        )
        ...
        if self.paint_step > 1:
            ...
            self.log_train_image(
                F.interpolate(cropped_rgb_render, (512, 512)) * (1 - checker_mask), "checkerboard_input"
            )
    ...
    def eval_render(self, data):
        ...
        outputs = self.mesh_model.render(theta=theta, phi=phi, radius=radius, dims=(dim, dim), background="white")
        z_normals = outputs["normals"][:, -1:, :, :].clamp(0, 1)
        rgb_render = outputs["image"]  # .permute(0, 2, 3, 1).contiguous().clamp(0, 1)
        diff = (
            (rgb_render.detach() - torch.tensor(self.mesh_model.default_color).view(1, 3, 1, 1).to(self.device))
            .abs()
            .sum(axis=1)
        )
        ...
        rgb_render = (
            ...
            + utils.color_with_shade([0.85, 0.85, 0.85], z_normals=z_normals, light_coef=0.3) * uncolored_mask
        )

        outputs_with_median = self.mesh_model.render(
            theta=theta, phi=phi, radius=radius, dims=(dim, dim), use_median=True, render_cache=outputs["render_cache"]
        )
    ...
    def calculate_trimap(
        ...
        diff = (
            (rgb_render_raw.detach() - torch.tensor(self.mesh_model.default_color).view(1, 3, 1, 1).to(self.device))
            .abs()
            .sum(axis=1)
        )
        ...
        # Extend mask
        generate_mask = (
            torch.from_numpy(cv2.dilate(exact_generate_mask[0, 0].detach().cpu().numpy(), np.ones((19, 19), np.uint8)))
        ...
        object_mask = (
            torch.from_numpy(cv2.erode(object_mask[0, 0].detach().cpu().numpy(), np.ones((7, 7), np.uint8)))
        ...
        if self.cfg.guide.initial_texture is None:
            ...
            refine_mask = (
                torch.from_numpy(cv2.dilate(refine_mask[0, 0].detach().cpu().numpy(), np.ones((31, 31), np.uint8)))
                .to(mask.device)
                .unsqueeze(0)
                .unsqueeze(0)
            )
        ...
        refine_mask = (
            torch.from_numpy(cv2.erode(refine_mask[0, 0].detach().cpu().numpy(), np.ones((5, 5), np.uint8)))
            .to(mask.device)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        refine_mask = (
            torch.from_numpy(cv2.dilate(refine_mask[0, 0].detach().cpu().numpy(), np.ones((5, 5), np.uint8)))
            .to(mask.device)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        if self.cfg.log.log_images:
            ...
            trimap_vis = (
                ...
                + utils.color_with_shade([255 / 255.0, 22 / 255.0, 67 / 255.0], z_normals=z_normals, light_coef=0.7)
                * exact_generate_mask
            )
            ...
            shaded_rgb_vis = (
                ...
                + utils.color_with_shade([0.85, 0.85, 0.85], z_normals=z_normals, light_coef=0.7) * exact_generate_mask
            )
            ...
            if self.paint_step > 1 or self.cfg.guide.initial_texture is not None:
                ...
                trimap_vis = trimap_vis * 0 + 1.0 * (
                    trimap_vis * (1 - only_old_mask_for_vis) + refinement_color_shaded * only_old_mask_for_vis
                )
    ...
    def generate_checkerboard(self, update_mask_inner, improve_z_mask_inner, update_mask_base_inner):
        ...
        only_old_mask = F.interpolate(
            torch.bitwise_and(improve_z_mask_inner == 1, update_mask_base_inner == 0).float(), (512, 512)
        )
    ...
    def project_back(
        ...
        object_mask = (
            torch.from_numpy(cv2.erode(object_mask[0, 0].detach().cpu().numpy(), np.ones((5, 5), np.uint8)))
            .to(object_mask.device)
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        blurred_render_update_mask = (
            torch.from_numpy(cv2.dilate(render_update_mask[0, 0].detach().cpu().numpy(), np.ones((25, 25), np.uint8)))
    ...
    def save_image(self, tensor: torch.Tensor, path: Path):
        if self.cfg.log.log_images:
            Image.fromarray(
                (einops.rearrange(tensor, "(1) c h w -> h w c").detach().cpu().numpy() * 255).astype(np.uint8)
            ).save(path)
